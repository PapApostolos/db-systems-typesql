    ...
    def gen_xc_type_batch(self, xc_type, is_col=False, is_list=False):
        ...
        for i, one_q in enumerate(xc_type):
            if is_list:
                q_val = []
                for item in one_q:
                    q_val.append(self.w2i.get(" ".join(sorted(item)), 0))
                # q_val = map(lambda x:self.w2i.get(" ".join(sorted(x)), 0), one_q)
            else:
                q_val = []
                for item in one_q:
                    q_val.append(self.w2i.get(item, 0))
    ...
    def gen_agg_batch(self, q):
        ...
        for b in range(B):
            if self.trainable:
                ct_val = []
                for item in agg_ops:
                    ct_val.append(self.w2i.get(item, 0))
                # ct_val = map(lambda x:self.w2i.get(x, 0), agg_ops)
            else:
                ct_val = []
                for item in agg_ops:
                    ct_val.append(self.word_emb.get(item, np.zeros(self.N_word, dtype=np.float32)))
