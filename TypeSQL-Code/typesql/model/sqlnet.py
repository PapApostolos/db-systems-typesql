...
class SQLNet(nn.Module):
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        loss = 0
        if pred_agg:
            agg_truth = [item[0] for item in truth_num]
            # agg_truth = map(lambda x:x[0], truth_num)
            data = torch.from_numpy(np.array(agg_truth))
            if self.gpu:
                agg_truth_var = Variable(data.cuda()).long()
            else:
                agg_truth_var = Variable(data).long()

            loss += self.CE(agg_score, agg_truth_var)

        if pred_sel:
            # sel_truth = map(lambda x:x[1], truth_num)
            sel_truth = [item[1] for item in truth_num]
            data = torch.from_numpy(np.array(sel_truth))
            if self.gpu:
                sel_truth_var = Variable(data.cuda()).long()
            else:
                sel_truth_var = Variable(data).long()
        ...
        if pred_cond:
            ...
            cond_num_truth = [item[2] for item in truth_num]

            data = torch.from_numpy(np.array(cond_num_truth))
            if self.gpu:
                cond_num_truth_var = Variable(data.cuda()).long()
            else:
                cond_num_truth_var = Variable(data).long()
            ...
            if self.gpu:
                cond_col_truth_var = Variable(data.cuda()).float()
            else:
                cond_col_truth_var = Variable(data).float()
            ...
            for b in range(len(truth_num)):
                ...
                if self.gpu:
                    cond_op_truth_var = Variable(data.cuda()).long()
                else:
                    cond_op_truth_var = Variable(data).long()
            ...
            for b in range(len(gt_where)):
                for idx in range(len(gt_where[b])):
                    ...
                    if self.gpu:
                        cond_str_truth_var = Variable(data.cuda()).long()
                    else:
                        cond_str_truth_var = Variable(data).long()
